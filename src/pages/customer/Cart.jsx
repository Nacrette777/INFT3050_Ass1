...
function Cart() {
  ...
  // The cart stores a snapshot of each item, so it renders immediately.
  // This background fetch refreshes price and stock from the database, in
  // case either changed after the item was added.
  ...
  useEffect(() => {
    ...
    getCatalogProducts()
      ...
      .catch(() => {
        // Non-fatal: fall back to the snapshot stored in the cart.
      });
    ...
    () =>
      cartItems.map((item) => {
        const product = catalog.find(
          (catalogItem) => getProductId(catalogItem) === item.id
        );
